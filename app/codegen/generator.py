...
WORKFLOW_MAIN_START = '''\
def _run():
    global _items, _final_output, _secret_store, _global_store, _results_table
'''
...
def _emit_waves(
    ...
    for w_idx, wave in enumerate(waves):
        if len(wave) == 1:
            ...
            lines.append(f"{pad}_node_start_ts = time.time()")
            ...
            if instrument:
                ...
                lines.append(f"{pad}    _node_end_ts = time.time()")
                lines.append(f"{pad}    _trace.append({{'id': {node_id!r}, 'type': {node_type!r}, 'label': {node_label!r}, 'status': 'ok', 'start_ts': _node_start_ts, 'end_ts': _node_end_ts, 'ts': _node_end_ts, 'items_in': _items_before, 'items_out': _items_after, 'debug': _node_debug}})")
                ...
                lines.append(f"{pad}    _node_end_ts = time.time()")
                lines.append(f"{pad}    _trace.append({{'id': {node_id!r}, 'type': {node_type!r}, 'label': {node_label!r}, 'status': 'stopped_current_execution', 'start_ts': _node_start_ts, 'end_ts': _node_end_ts, 'ts': _node_end_ts, 'items_in': _items_before, 'error': _stop_ex.message}})")
                lines.append(f"{pad}    raise")
                lines.append(f"{pad}except Exception as _tr_ex:")
                lines.append(f"{pad}    _node_end_ts = time.time()")
                lines.append(f"{pad}    _trace.append({{'id': {node_id!r}, 'type': {node_type!r}, 'label': {node_label!r}, 'status': 'error', 'start_ts': _node_start_ts, 'end_ts': _node_end_ts, 'ts': _node_end_ts, 'items_in': _items_before, 'error': str(_tr_ex)}})")
                lines.append(f"{pad}    raise")
                lines.append("")
            else:
                if emit_results_table:
                    lines.append(f"{pad}try:")
                    lines.append(node.to_code(indent=base_indent + 4))
                    lines.append(f"{pad}    _items_after = list(_items)")
                    lines.append(f"{pad}    _node_outputs_raw = _branch_outputs if isinstance(_branch_outputs, dict) else {{'output_1': _items_after}}")
                    ...
                    lines.append(f"{pad}    _node_outputs[{node_id!r}] = _node_outputs_norm")
                    lines.append(f"{pad}    _node_end_ts = time.time()")
                    lines.append(f"{pad}    _append_results_table_row({{'index': len(_results_table) + 1, 'id': {node_id!r}, 'label': {node_label!r}, 'type': {node_type!r}, 'start_ts': _node_start_ts, 'end_ts': _node_end_ts, 'ts': _node_end_ts, 'status': 'ok', 'items_in': _items_before, 'items_out': _items_after, 'outputs': _node_outputs_norm, 'iteration': EXECUTION_ID}})")
                    lines.append(f"{pad}except _StopIterationExecution as _stop_ex:")
                    lines.append(f"{pad}    _node_end_ts = time.time()")
                    lines.append(f"{pad}    _append_results_table_row({{'index': len(_results_table) + 1, 'id': {node_id!r}, 'label': {node_label!r}, 'type': {node_type!r}, 'start_ts': _node_start_ts, 'end_ts': _node_end_ts, 'ts': _node_end_ts, 'status': 'stopped_current_execution', 'items_in': _items_before, 'error': _stop_ex.message, 'iteration': EXECUTION_ID}})")
                    lines.append(f"{pad}    raise")
                    lines.append(f"{pad}except Exception as _dbg_ex:")
                    lines.append(f"{pad}    _node_end_ts = time.time()")
                    lines.append(f"{pad}    _append_results_table_row({{'index': len(_results_table) + 1, 'id': {node_id!r}, 'label': {node_label!r}, 'type': {node_type!r}, 'start_ts': _node_start_ts, 'end_ts': _node_end_ts, 'ts': _node_end_ts, 'status': 'error', 'items_in': _items_before, 'error': str(_dbg_ex), 'iteration': EXECUTION_ID}})")
                    lines.append(f"{pad}    raise")
                    lines.append("")
                else:
                    lines.append(node.to_code(indent=base_indent))
            ...
            for b_idx, node_data in enumerate(wave):
                ...
                lines.append(f"{inner_pad}_node_start_ts = time.time()")
                ...
                if instrument:
                    ...
                    lines.append(f"{inner_pad}    _node_end_ts = time.time()")
                    lines.append(f"{inner_pad}    _trace.append({{'id': {node_id!r}, 'type': {node_type!r}, 'label': {node_label!r}, 'status': 'ok', 'start_ts': _node_start_ts, 'end_ts': _node_end_ts, 'ts': _node_end_ts, 'items_in': _items_before, 'items_out': _items_after, 'debug': _node_debug}})")
                    ...
                    lines.append(f"{inner_pad}    _node_end_ts = time.time()")
                    lines.append(f"{inner_pad}    _trace.append({{'id': {node_id!r}, 'type': {node_type!r}, 'label': {node_label!r}, 'status': 'stopped_current_execution', 'start_ts': _node_start_ts, 'end_ts': _node_end_ts, 'ts': _node_end_ts, 'items_in': _items_before, 'error': _stop_ex.message}})")
                    lines.append(f"{inner_pad}    raise")
                    lines.append(f"{inner_pad}except Exception as _tr_ex:")
                    lines.append(f"{inner_pad}    _node_end_ts = time.time()")
                    lines.append(f"{inner_pad}    _trace.append({{'id': {node_id!r}, 'type': {node_type!r}, 'label': {node_label!r}, 'status': 'error', 'start_ts': _node_start_ts, 'end_ts': _node_end_ts, 'ts': _node_end_ts, 'items_in': _items_before, 'error': str(_tr_ex)}})")
                    lines.append(f"{inner_pad}    raise")
                else:
                    if emit_results_table:
                        lines.append(f"{inner_pad}try:")
                        ...
                        lines.append(f"{inner_pad}    _wave_{w_idx}_results[{b_idx}] = {{'items': _items_after, 'outputs': _node_outputs_norm, 'debug': _node_debug}}")
                        lines.append(f"{inner_pad}    _node_end_ts = time.time()")
                        lines.append(f"{inner_pad}    _append_results_table_row({{'index': len(_results_table) + 1, 'id': {node_id!r}, 'label': {node_label!r}, 'type': {node_type!r}, 'start_ts': _node_start_ts, 'end_ts': _node_end_ts, 'ts': _node_end_ts, 'status': 'ok', 'items_in': _items_before, 'items_out': _items_after, 'outputs': _node_outputs_norm, 'iteration': EXECUTION_ID}})")
                        lines.append(f"{inner_pad}except _StopIterationExecution as _stop_ex:")
                        lines.append(f"{inner_pad}    _node_end_ts = time.time()")
                        lines.append(f"{inner_pad}    _append_results_table_row({{'index': len(_results_table) + 1, 'id': {node_id!r}, 'label': {node_label!r}, 'type': {node_type!r}, 'start_ts': _node_start_ts, 'end_ts': _node_end_ts, 'ts': _node_end_ts, 'status': 'stopped_current_execution', 'items_in': _items_before, 'error': _stop_ex.message, 'iteration': EXECUTION_ID}})")
                        lines.append(f"{inner_pad}    raise")
                        lines.append(f"{inner_pad}except Exception as _dbg_ex:")
                        lines.append(f"{inner_pad}    _node_end_ts = time.time()")
                        lines.append(f"{inner_pad}    _append_results_table_row({{'index': len(_results_table) + 1, 'id': {node_id!r}, 'label': {node_label!r}, 'type': {node_type!r}, 'start_ts': _node_start_ts, 'end_ts': _node_end_ts, 'ts': _node_end_ts, 'status': 'error', 'items_in': _items_before, 'error': str(_dbg_ex), 'iteration': EXECUTION_ID}})")
                        lines.append(f"{inner_pad}    raise")
                    else:
                        lines.append(node.to_code(indent=base_indent + 4))
                        lines.append(f"{inner_pad}_items_after = list(_items)")
...
def generate_script(
    ...
    lines: list[str] = [
        ...
        f'WORKFLOW_DEBUG = {"True" if debug else "False"}\n',
    ]
    ...
    if scheduler_wave_idx is not None:
        ...
        # Close the loop with time.sleep (indent=8, inside while True)
        lines.append("        time.sleep(_sleep_seconds)")
        lines.append("")
...
def generate_run_script(workflow_name: str, workflow_id: str, nodes: list[dict], edges: list[dict]) -> str:
    ...
    if trigger_node["type"] == WebhookNode.NODE_TYPE:
        ...
        lines.append("                _webhook_now_ts = time.time()")
        lines.append("                _trace.append({'id': webhook_node_id, 'type': 'webhook', 'label': webhook_node_label, 'status': 'stopped_current_execution', 'error': 'Run stopped by user', 'start_ts': _webhook_now_ts, 'end_ts': _webhook_now_ts, 'ts': _webhook_now_ts, 'items_in': [], 'items_out': []})")
        ...
        lines.append("                return")
        lines.append("            _webhook_start_ts = time.time()")
        lines.append("            try:")
        ...
        lines.append("                }")
        lines.append("                _webhook_end_ts = time.time()")
        lines.append("                _trace.append({'id': webhook_node_id, 'type': 'webhook', 'label': webhook_node_label, 'status': 'ok', 'start_ts': _webhook_start_ts, 'end_ts': _webhook_end_ts, 'ts': _webhook_end_ts, 'items_in': [], 'items_out': [{'_webhook_request': _request_payload}]})")
        ...
        lines.append("                _webhook_end_ts = time.time()")
        lines.append("                _trace.append({'id': webhook_node_id, 'type': 'webhook', 'label': webhook_node_label, 'status': 'error', 'error': str(_webhook_ex), 'start_ts': _webhook_start_ts, 'end_ts': _webhook_end_ts, 'ts': _webhook_end_ts, 'items_in': [], 'items_out': []})")
        ...
        lines.append("        else:")
        lines.append("            _webhook_now_ts = time.time()")
        lines.append("            _trace.append({'id': webhook_node_id, 'type': 'webhook', 'label': webhook_node_label, 'status': 'error', 'error': f'Webhook timeout after {_timeout_total}s', 'start_ts': _webhook_now_ts, 'end_ts': _webhook_now_ts, 'ts': _webhook_now_ts, 'items_in': [], 'items_out': []})")
    ...
    if scheduler_wave_idx is not None:
        ...
        lines.append("    _scheduler_start_ts = time.time()")
        lines.append("    try:")
        lines.append("        for _run_iter in range(1):")
        lines.append("            _scheduler_end_ts = time.time()")
        lines.append(f"            _trace.append({{'id': {node_id!r}, 'type': {node_type!r}, 'label': {node_label!r}, 'status': 'ok', 'start_ts': _scheduler_start_ts, 'end_ts': _scheduler_end_ts, 'ts': _scheduler_end_ts, 'items_in': _items_before, 'items_out': list(_items)}})")
        ...
        lines.append("    except Exception as _tr_ex:")
        lines.append("        _scheduler_end_ts = time.time()")
        lines.append(f"        _trace.append({{'id': {node_id!r}, 'type': {node_type!r}, 'label': {node_label!r}, 'status': 'error', 'start_ts': _scheduler_start_ts, 'end_ts': _scheduler_end_ts, 'ts': _scheduler_end_ts, 'items_in': _items_before, 'error': str(_tr_ex)}})")
        lines.append("        raise")
        lines.append("")
    else:
        lines.append("    try:")
