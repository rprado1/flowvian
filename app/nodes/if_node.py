...
class IfNode(BaseNode):
    ...
    def to_code(self, indent: int = 0) -> str:
        ...
        lines = [
            ...
            "    _if_item_eval = {'item_index': len(_if_evaluations), 'conditions': [], 'final_result': False}",
            "    _if_aggregate = None",
            ...
            "",
            "        _if_left_exists = _if_input_name in _item",
            "        _if_left = _item.get(_if_input_name)",
            "        _if_left_repr = _if_left",
            "        _if_needs_compare = _if_operator not in _if_unary_ops",
            "        _if_right = None",
            "",
            "        if (not _if_left_exists) or (_if_left is None):",
            "            _if_result = False",
            ...
            "",
            "            _if_item_eval['conditions'].append({",
            "                'variable': '${' + _if_input_name + '}',",
            "                'resolved_value': _if_left_repr,",
            "                'operator': _if_operator,",
            "                'compare_value': _if_compare_raw if _if_needs_compare else None,",
            "                'resolved_compare': None,",
            "                'join': None if _if_idx == 0 else _if_join.upper(),",
            "                'result': False,",
            "            })",
            "            continue",
            ...
            "            'operator': _if_operator,",
            "            'compare_value': _if_compare_raw if _if_needs_compare else None,",
            "            'resolved_compare': _if_right if _if_needs_compare else None,",
            "            'join': None if _if_idx == 0 else _if_join.upper(),",
            "            'result': bool(_if_result),",
            "        })",
            "",
            "    _if_item_eval['final_result'] = bool(_if_aggregate)",
            "    _if_evaluations.append(_if_item_eval)",
            "",
            ...
            "_node_debug = {'if_evaluations': _if_evaluations, 'branch_counts': {'true': len(_items_true), 'false': len(_items_false)}}",
